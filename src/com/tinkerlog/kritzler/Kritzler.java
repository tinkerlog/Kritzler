...
public class Kritzler {
	 ...
	 * @param parent PApplet object used by main sketch
	 * @param port Serial port set up in main sketch
	 */
	 ...
	 * 
	 * Values should correspond to coordinate of top-left corner of physical canvas.
	 * 
	 * @param x	X offset
	 * @param y	Y offset
	 */
	 ...
	 * 
	 * @param s	Scale factor (1 = no scaling)
	 */
	...
	public void processSerial() {
		while (port.available() > 0) {
			...
			// Process buffer
				...
				// Retroeve complete message from Serial buffer, then reset buffer
				...
				if (message.startsWith("#")) {
					...
					if(message.startsWith("#idle, xy:")) {
						...
						this.botX = Float.valueOf(tokens[0]) - tx;
						this.botY = Float.valueOf(tokens[1]) - ty;
					}
				} else {
	 ...
	 * 
	 * @param plotting	true for plotting, false for no activity
	 */
